def get_result(file_path):
    ...
    print(dict)
    print(times)
    result = sub_func("start", dict, times)
...
def sub_func(current_node, dict, times):
    if current_node == "end":
        return 1

    if current_node != "start" and current_node.islower():
        if times[current_node] > 0:
            return 0
        else:
    ...
    for node in dict[current_node]:
        result += sub_func(node, dict, times)

    if current_node != "start" and current_node.islower():
        times[current_node] = 0
